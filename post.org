...
CLOSED: [2019-07-15 Mon 00:16]
   ...
   :END:
$h_s$ and $h_t$ are source hidden states and target hidden state, the shape is =(n,1)=. $c_t$ is the final context vector, and $\alpha_{t,s}$ is alignment score.

$$\begin{aligned}
c_t&=\sum_{i=1}^n \alpha_{t,s}h_i \\
\alpha_{t,s}&= \frac{\exp(score(s_t,h_s))}{\sum_{i=1}^n \exp(score(s_t,h_i))}
\end{aligned}
$$
...
****** Dot-Product
$$score(s_t,h_s)=s_t^Th_s$$

****** Scaled Dot-Product
$$score(s_t,h_s)=\frac{s_t^Th_s}{\sqrt{n}}$$
Google's Transformer has similar scaling factor: $score=\frac{KQ^T}{\sqrt{n}}$

****** Location-Base

$$socre(s_t,h_s)=softmax(W_as_t)$$

****** General

$$score(s_t,h_s)=s_t^TW_ah_s$$

$Wa$ 'shape is =(n,n)=

****** Concat

$$score(s_t,h_s)=v_a^Ttanh(W_a[s_t,h_s])$$

$v_a$ 'shape is =(x,1)=, and $Wa$ 's shape is =(x,x)=. This is similar to a neural network with one hidden layer.
