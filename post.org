...
&=PE_{pos\_even}A+PE_{pos\_odd}B
\end{aligned}$$
...
Let's begin with single head attention. In short, it use a K Q V matrix to calculate the attention.
...
Each head has its own $W^Q$ $W^K$ $W^V$ matrix, and produces $Z_0,Z_1...Z_7$,($Z_0$'s shape is =(SEQ_LEN, 64)=) the concat the outputted vectors as $O$. $O$ will multiply a weight matrix $W^O$ ($W^O$'s shape is =(512, 512)=) and the result is $Z$, which will be sent to Feed Forward Network.
...
Layer Norm is similar to Batch Normalization, but it tries to normalize the whole layer rather than each feature. After normalization, each layer's output(=512*SEQ_LEN= vector) has the same mean and standard deviation. More details can be found in this [[https://arxiv.org/abs/1607.06450][paper]].
...
5. [[http://blog.stupidme.me/transformer-attention-is-all-you-need/][Transformer模型的PyTorch实现]]
