...
- [[https://github.com/facebookresearch/visdom][Visdom]]
  ...
  It's a visualization tool made by FaceBook. It's more convenient to use it rather than generate graph manually by matplotlib. Besides =reward= and =mean_q=, these variable are also useful when debugging: total frames, TD-error, loss and action_distribution.
